...
import { GameState, GameActions, Player } from '../types';
...
export const useGameStore = create<GameState & GameActions>((set, get) => ({
  ...
  setSelectedCategory: (category: any) => set({ selectedCategory: category }),

  setQuestionType: (type: any) => set({ questionType: type }),
  ...
  incrementCategoryCount: (playerId: any, category: string | undefined, completed: any) => set((state: { players: any[]; }) => ({
    players: state.players.map((p) => {
      if (p.id === playerId) {
